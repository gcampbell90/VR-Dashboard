...
public class EarthquakeManager : MonoBehaviour, IDataLoader {
    ...
    public async void ParseFile(string fileText) {
        ...
        //replace latlong
        for (int i = 0; i < csvReader.LocationData.Count; i++)
        {
                var lng = Convert.ToSingle(csvReader.LocationData[i].Longitude);
                var lat = Convert.ToSingle(csvReader.LocationData[i].Latitude);
                ...
                //var plotted = eP.PlotFlat(transform, earthquakesContainer.transform, LayerMask.NameToLayer("Location"));
